...
volatile uint8_t keypadChangeState = 0;  // 0 no change, 1 change
...
int main() {
    ...
    if (EEPROM_read(4) == 0) {
        ...
        // write to EEPROM if the current EEPROM has no initial write at ADDRESS
        // 0.
        for (int i = 0; i < 4; i++) {
            EEPROM_write(i, code[i]);
        }
        EEPROM_write(4, 1);  // write 4th address if there is already a PIN
                             // ready. Else, it's already written
    }
    ...
    while (1) {
        ...
        if (alarmState == 0) {           // if alarm is off
            if (ADC > 50) {              // if laser is blocked
                ...
                // TODO: Ask for original PIN
                // TODO: Ask for new PIN
                // TODO: Write to EEPROM new pin
                if (keypadChangeState == 1) {
                    if (pinChange(keyPressed)) {
                        ...
                        while (currentDigit != 10) {
                            ...
                            if (keyPressed == 255) {
                                ...
                                lq_clear(&device);
                                lq_setCursor(&device, 1, 0);
                                lq_print(&device, "Writing to EEPROM...");
                                for (int i = 0; i < 4; i++) {
                                    EEPROM_write(i, newBuffer[i]);
                                }
                                keypadChangeState = 0;  // return to monitoring
                            } else if (keyPressed == 253) {
                                if (currentDigit == 0) {
                                    _delay_ms(100);
                                } else {
                                    currentDigit--;
                                    lq_setCursor(&device, 1, currentDigit + 4);
                                    lq_print(&device, " ");
                                    _delay_ms(250);
                                }
                            } else {
                                newBuffer[currentDigit] = keyPressed;
...
bool pinChange(uint8_t keyPressed) {
    uint8_t buffer[4];
    uint8_t digitC = 0;
    while (digitC != 10) {
        char keypad[4];
